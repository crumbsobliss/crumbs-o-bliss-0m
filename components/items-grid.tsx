...
export function ItemsGrid({ items, locale }: ItemsGridProps) {
  ...
  const toggleFavorite = (e: React.MouseEvent, slug: string) => {
    ...
    setFavorites(prev => {
      ...
      if (newFavs.has(slug)) newFavs.delete(slug)
      else newFavs.add(slug)
      return newFavs
    })
  }
        ...
        <div className="max-w-[1800px] mx-auto p-4 md:p-6 space-y-4">
          ...
          <div className="flex flex-col lg:flex-row gap-4 items-end lg:items-center justify-between">
            ...
            <div className="w-full lg:w-1/3 relative group">
              <input
                type="text"
                placeholder={locale === "en" ? "Search our collection..." : "অনুসন্ধান করুন..."}
                value={searchQuery}
              ...
              {searchQuery && (
                <button onClick={() => setSearchQuery("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-[var(--muted-foreground)] hover:text-[var(--primary)]">
                  <X className="h-4 w-4" />
                </button>
              )}
            </div>
            ...
            {hasActiveFilters && (
              ...
              >
                {selectedTags.map((tag) => (
                  <span key={tag} className="inline-flex items-center gap-1.5 px-3 py-1 bg-[var(--accent)] text-[var(--accent-foreground)] rounded-full text-sm font-medium shadow-sm">
                    {tag}
                    <button onClick={() => toggleTag(tag)} className="hover:bg-black/20 rounded-full p-0.5"><X className="h-3 w-3" /></button>
                  </span>
                ))}
                <button onClick={resetFilters} className="text-[var(--destructive)] text-xs hover:underline px-2 font-bold">Clear All</button>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
          ...
          {showFilters && (
            ...
            >
              <div className="max-w-[1800px] mx-auto p-4 md:p-6">
                <p className="text-[var(--muted-foreground)] font-cinzel text-xs tracking-widest mb-3 uppercase">Filter by Category</p>
          ...
          <AnimatePresence mode="popLayout">
            {filteredItems.length > 0 ? (
                          ...
                          <button 
                            ...
                            className={`p-2 rounded-full backdrop-blur-sm transition-all ${
                              favorites.has(item.slug) 
                                ...
                                : "bg-[var(--background)]/80 text-[var(--muted-foreground)] hover:text-[var(--destructive)]"
                            }`}
                          >
                            <Heart className={`w-4 h-4 ${favorites.has(item.slug) ? "fill-current" : ""}`} />
                          </button>
                        </div>
                      </div>
                      ...
                      <div className="p-5 flex flex-col sm:flex-row flex-grow gap-4 justify-between">
                         <div className="space-y-2 flex-grow">
                            <div className="flex items-center gap-3">
                              ...
                              {favorites.has(item.slug) && <Heart className="w-4 h-4 text-[var(--destructive)] fill-current" />}
                            </div>
                            ...
                            <div className="flex flex-wrap gap-2 mt-2">
                               {item.tags.map(t => (
                                 <span key={t} className="text-xs bg-[var(--secondary)] text-[var(--secondary-foreground)] px-2 py-1 rounded-md uppercase font-cinzel">{t}</span>
                               ))}
                            </div>
                         </div>
                         <div className="flex sm:flex-col justify-between items-end gap-4 shrink-0 border-t sm:border-t-0 sm:border-l border-[var(--border)] pt-4 sm:pt-0 sm:pl-6">
                            <span className="font-prata text-2xl text-[var(--highlight)] font-bold">₹{item.price}</span>
              ...
              <motion.div className="col-span-full py-20 text-center space-y-4">
                 ...
                 <h3 className="font-playfair text-3xl text-[var(--text)]">No delicacies found</h3>
                 <p className="font-cormorant text-xl text-[var(--muted-foreground)]">Try adjusting your filters or search terms.</p>
                 <button onClick={resetFilters} className="text-[var(--highlight)] font-bold underline underline-offset-4 hover:text-[var(--accent)]">Clear all filters</button>
              </motion.div>
            )}
          </AnimatePresence>
        </motion.div>
            ...
            >
               ...
               <div className="w-full md:w-1/2 p-6 md:p-10 flex flex-col overflow-y-auto">
                 <div className="flex justify-between items-start mb-4">
                    <div className="space-y-1">
                      <h2 className="font-playfair text-3xl font-bold text-[var(--text)]">
                        {locale === "en" ? selectedItem.name.en : selectedItem.name.bn}
                      </h2>
                      <div className="flex items-center gap-2">
                        {selectedItem.tags.map(t => (
                           <span key={t} className="text-[10px] font-bold bg-[var(--secondary)] text-[var(--secondary-foreground)] px-2 py-0.5 rounded-sm uppercase tracking-wider">{t}</span>
                        ))}
                      </div>
                    </div>
                    <button 
                 ...
                 <p className="font-cormorant text-xl text-[var(--muted-foreground)] leading-relaxed mb-8">
                   {locale === "en" ? selectedItem.description.en : selectedItem.description.bn}
                 </p>

                 {/* Stats */}
                 <div className="grid grid-cols-2 gap-4 mb-8">
                    <div className="bg-[var(--secondary)]/50 p-4 rounded-lg border border-[var(--border)]">
                       <span className="block text-xs font-cinzel text-[var(--muted-foreground)] uppercase mb-1">Calories</span>
                       <span className="block font-old-standard text-2xl text-[var(--text)]">{selectedItem.calories}</span>
                    </div>
                    <div className="bg-[var(--secondary)]/50 p-4 rounded-lg border border-[var(--border)]">
                       <span className="block text-xs font-cinzel text-[var(--muted-foreground)] uppercase mb-1">Price</span>
                       <span className="block font-prata text-2xl text-[var(--highlight)]">₹{selectedItem.price}</span>
                    </div>
                 </div>
                 ...
                 <div className="mt-auto flex gap-4">
                    <button 
                      onClick={(e) => toggleFavorite(e, selectedItem.slug)}
                      className={`flex-1 py-3 rounded-[var(--radius)] border border-[var(--border)] font-cinzel text-sm uppercase tracking-widest flex items-center justify-center gap-2 transition-all ${
                        favorites.has(selectedItem.slug)
                          ? "bg-[var(--destructive)] border-[var(--destructive)] text-white"
                          : "hover:bg-[var(--secondary)] text-[var(--text)]"
                      }`}
                    >
                      <Heart className={`w-4 h-4 ${favorites.has(selectedItem.slug) ? "fill-current" : ""}`} />
                      {favorites.has(selectedItem.slug) ? "Saved" : "Save"}
                    </button>
                    <Link
